"""Password-based image encryption using the same AES-256-GCM + Scrypt approach."""
...
class PicCryptoError(Exception):
    """Raised when an image cannot be encrypted or decrypted."""
...
def _decode(value: object, name: str) -> bytes:
    if not isinstance(value, str):
        raise PicCryptoError(f"加密文件中的 {name} 字段无效")
    try:
        return base64.b64decode(value.encode("ascii"), validate=True)
    except (UnicodeEncodeError, binascii.Error) as exc:
        raise PicCryptoError(f"加密文件中的 {name} 字段无效") from exc


def encrypt_bytes(plaintext: bytes, password: str) -> bytes:
    """Encrypt bytes into the versioned JSON envelope."""
...
def decrypt_bytes(encrypted: bytes, password: str) -> bytes:
    ...
    if len(salt) != _SALT_LENGTH or len(nonce) != _NONCE_LENGTH:
        raise PicCryptoError("加密文件参数无效")

    key = _derive_key(password, salt)
    try:
        return AESGCM(key).decrypt(nonce, ciphertext, _header_bytes())
    except InvalidTag as exc:
        raise PicCryptoError("密码错误或加密文件已被篡改") from exc


def _atomic_write(path: Path, content: bytes) -> None:
    path = Path(path)
    if not path.parent.exists():
        raise PicCryptoError(f"目标目录不存在: {path.parent}")
    ...
    try:
        ...
        raise PicCryptoError(f"无法写入文件 {path}: {exc}") from exc
    finally:
...
def encrypt_file(source: Path, target: Path, password: str) -> None:
    ...
    try:
        ...
        raise PicCryptoError(f"无法读取文件 {source}: {exc}") from exc
