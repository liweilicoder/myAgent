...
class ProjectAssistant(JosieSimpleAgent):
    ...
    def run(self, user_input: str, note_as_action: bool = False) -> str:
        ...
        response = self.llm.invoke(messages, trim_think=True)
...
def main():
    log.delimiter("=" * 80)
    log.delimiter("NoteTool 与 ContextBuilder 集成示例")
    log.delimiter("=" * 80 + "\n")
