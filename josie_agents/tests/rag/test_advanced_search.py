...
load_dotenv()
DemoNamespace="advanced_search_demo"
...
"""
        ...
        for doc in tech_documents:
            result = self.rag_tool.run({"action": "add_text",
                                        ...
                                        "chunk_overlap": 10,
                                        "document_id": doc["id"]})
            log.test(f"✅ 添加文档: {doc['id']}")

        log.test(f"📊 知识库设置完成，共添加 {len(tech_documents)} 个文档")

    def test_basic_search(self):
        """演示基础搜索功能"""
        log.delimiter(" 🔍 基础搜索功能演示")
        log.delimiter("-" * 50)
        ...
        for query, description in basic_queries:
            ...
            log.test(f"耗时: {search_time:.3f}秒")
            log.test(f"结果: {result}")

    def test_mqe_search(self):
        """演示多查询扩展（MQE）搜索"""
        log.delimiter(" 🔄 多查询扩展（MQE）搜索演示")
        log.delimiter("-" * 50)

        log.test("MQE搜索原理:")
        log.test("• 🤖 使用LLM生成语义等价查询")
        log.test("• 🔍 并行执行多个查询")
        log.test("• 📊 合并和去重结果")
        log.test("• 🎯 提高召回率和覆盖面")
    ...
    def test_hyde_search(self):
        """演示假设文档嵌入（HyDE）搜索"""
        log.delimiter(" 📝 假设文档嵌入（HyDE）搜索演示")
        log.delimiter("-" * 50)

        log.test("HyDE搜索原理:")
        log.test("• 🤖 LLM生成假设性答案文档")
        log.test("• 📄 将假设文档作为查询向量")
        log.test("• 🎯 改善查询-文档匹配效果")
        log.test("• 🔍 特别适合复杂问题检索")
    ...
    def test_combined_advanced_search(self):
        """演示组合高级搜索"""
        log.delimiter("🚀 组合高级搜索演示")
        log.delimiter("-" * 50)

        log.test("组合搜索策略:")
        log.test("• 🔄 MQE + HyDE 双重扩展")
        log.test("• 📊 多策略结果融合")
        log.test("• 🎯 最大化检索效果")
        log.test("• ⚡ 智能缓存优化")
    ...
    def test_search_performance_analysis(self):
        ...
        log.test(f"基础搜索平均耗时: {basic_avg:.3f}秒")
        log.test(f"高级搜索平均耗时: {advanced_avg:.3f}秒")
        log.test(f"性能比值: {advanced_avg / basic_avg:.1f}x")
...
def main():
    """主函数"""
    log.delimiter("🚀 RAGTool高级检索策略演示")
    log.delimiter("展示MQE、HyDE等先进检索技术的实现和应用")
    log.delimiter("=" * 70)
