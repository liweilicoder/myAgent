...
class MarkitdownPipelineDemo:
    ...
    def __init__(self):
        self.rag_tool = RAGTool(
            ...
            rag_namespace=DemoNamespace
        )
        self.temp_dir = tempfile.mkdtemp(dir="./demo_rag_kb")
    ...
    def test_markitdown_conversion(self, file_paths):
        """演示MarkItDown转换过程"""
        log.delimiter(" 🔄 MarkItDown转换过程演示")
        log.delimiter("-" * 50)

        log.test("MarkItDown处理流程:")
        log.test("1. 📄 检测文档格式")
        log.test("2. 🔄 转换为Markdown")
        log.test("3. 📝 保持结构信息")
        log.test("4. ✨ 统一格式输出")
        ...
        for filename, file_path in file_paths.items():
            log.test(f"处理文档: {filename}")
            log.test(f"原始格式: {os.path.splitext(filename)[1]}")
            ...
            log.test(f"处理结果: {result}")
            log.test(f"处理时间: {process_time:.3f}秒")
            log.test(f"✅ {filename} → Markdown → 分块 → 向量化")
    ...
    def test_markdown_chunking(self):
        """演示基于Markdown的智能分块"""
        log.delimiter("📊 基于Markdown的智能分块演示")
        log.delimiter("-" * 50)

        log.test("Markdown分块策略:")
        log.test("• 🏷️ 标题层次感知 - 利用#、##、###结构")
        log.test("• 📝 段落语义保持 - 保持内容完整性")
        log.test("• 🔢 Token精确控制 - 适配嵌入模型")
        log.test("• 🔗 智能重叠策略 - 避免信息丢失")
...
"""
        ...
        log.test(f" 📝 添加复杂Markdown文档进行分块测试...")
        ...
        log.test(f"分块结果: {result}")

        # 测试基于结构的检索
        log.test(f" 🔍 测试基于Markdown结构的检索:")
    ...
    def test_embedding_optimization(self):
        """演示面向嵌入的Markdown预处理"""
        log.delimiter(" 🎯 面向嵌入的Markdown预处理演示")
        log.delimiter("-" * 50)

        log.test("Markdown预处理优化:")
        log.test("• 🏷️ 移除格式标记，保留语义内容")
        log.test("• 🔗 处理链接格式，保留链接文本")
        log.test("• 💻 清理代码块，保留代码内容")
        log.test("• 🧹 清理多余空白，优化向量表示")
...
"""
        ...
        log.test(f" 📝 原始Markdown内容:")
        log.test(raw_markdown)
    ...
    def test_pipeline_performance(self):
        ...
        log.test(f"批量处理结果: {batch_result}")
        log.test(f"总耗时: {batch_time:.3f}秒")
        log.test(f"平均每文档: {batch_time / 10:.3f}秒")

        # 获取最终统计
        stats = self.rag_tool.run({"action": "stats"})
        log.test(f" 📊 最终统计: {stats}")


def main():
    """主函数"""
    log.delimiter("🔄 RAGTool的MarkItDown处理管道演示")
    log.delimiter("展示Any格式→Markdown→分块→向量化的完整流程")
    log.delimiter("=" * 70)
    ...
    try:
        ...
        log.test(f" 🧹 清理临时文件: {demo.temp_dir}")
