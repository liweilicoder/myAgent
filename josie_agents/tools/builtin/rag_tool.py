...
from josie_agents.memory.rag.pipeline import create_rag_pipeline
...
class RAGTool(BaseTool):
    ...
    def _init_components(self):
        ...
        try:
            ...
            log.success(f"✅ RAG工具初始化成功: namespace={self.rag_namespace}, collection={self.collection_name}")

        except Exception as e:
            self.initialized = False
            self.init_error = str(e)
            log.error(f"❌ RAG工具初始化失败: {e}")
    ...
    def _ask(self, question: Optional[str] = None, query: Optional[str] = None, limit: int = 5,
        ...
        try:
            ...
            # 5. 调用 LLM 生成答案
            llm_start = time.time()
            answer = self.llm.invoke(enhanced_prompt, trim_think=True)
    ...
    def add_documents_batch(self, file_paths: List[str], namespace: str = "default") -> str:
        ...
        for i, file_path in enumerate(file_paths, 1):
            log.info(f"📄 处理文档 {i}/{len(file_paths)}: {os.path.basename(file_path)}")
    ...
    def add_texts_batch(self, texts: List[str], namespace: str = "default",
        ...
        for i, text in enumerate(texts):
            ...
            log.info(f"📝 处理文本 {i + 1}/{len(texts)}: {doc_id}")
