...
class ContextBuilder:
        ...
        if self.memory_tool:
            try:
                ...
                if state_results and "未找到" not in state_results:
                    log.debug(f" ✋[ContextBuilder][memory_gather] (任务状态 OR 子目标 OR 结论 OR 阻塞) ret={state_results}")
                    packets.append(ContextPacket(
                ...
                if related_results and "未找到" not in related_results:
                    log.debug(f" ✋[ContextBuilder][memory_gather] related_memory, ret={related_results}")
                    packets.append(ContextPacket(
                        content=related_results,
                        metadata={"type": "related_memory"}
                    ))
            except Exception as e:
                log.error(f"⚠️ 记忆检索失败: {e}")
        ...
        if self.rag_tool:
            try:
                ...
                if rag_results and "未找到" not in rag_results and "错误" not in rag_results:
                    log.debug(f" ✋[ContextBuilder][rag_gather] rag_results={rag_results}")
                    packets.append(ContextPacket(
                        content=rag_results,
                        metadata={"type": "knowledge_base"}
                    ))
            except Exception as e:
                log.error(f"⚠️ RAG检索失败: {e}")
    ...
    def _select(
        ...
        for p in packets:
            ...
            log.debug(f" ✋[ContextBuilder][score_calculate] content={p.content[:100]}..., score={score}, type={p.metadata['type']}"
                      f"\n\t score({score})=0.7*relevance_score({p.relevance_score})+0.3*recency_score({rec})")
    ...
    def _compress(self, context: str) -> str:
        ...
        if current_tokens <= available_tokens:
            log.debug(f" ✋[ContextBuilder][compress] compress not triggered, current_tokens={current_tokens}, available_tokens={available_tokens}")
            return context

        # 简单截断策略（保留前N个token）
        # 实际应用中可用LLM做高保真摘要
        log.debug(f" ⚠️[ContextBuilder][compress] 上下文超预算 ({current_tokens} > {available_tokens})，执行截断")
