...
class MCPServer:
    ...
    def run(self, transport: str = "stdio", **kwargs):
        ...
        """
        log.info(f"🔌 MCP Server:{self.name} start!")
...
if __name__ == "__main__":
    ...
    log.delimiter(f"🚀 Starting {server.name}...")
    log.test(f"📝 {server.description}")
    log.test(f"🔌 Protocol: MCP")
    log.test(f"📡 Transport: stdio")
    server.run()