...
class MCPClient:
    ...
    def _prepare_server_source(self, server_source: Union[str, List[str], FastMCP, Dict[str, Any]]):
        ...
        if isinstance(server_source, FastMCP):
            log.debug(f"🧠 使用内存传输: {server_source.name}")
            return server_source

        # 2. 配置字典 - 根据配置创建传输
        if isinstance(server_source, dict):
            log.debug(f"⚙️ 使用配置传输: {server_source.get('transport', 'stdio')}")
        ...
        if isinstance(server_source, str) and (
            ...
            log.debug(f"🌐 使用 {transport_type.upper()} 传输: {server_source}")
        ...
        if isinstance(server_source, str) and server_source.endswith(".py"):
            log.debug(f"🐍 使用 Stdio 传输 (Python): {server_source}")
        ...
        if isinstance(server_source, list) and len(server_source) >= 1:
            log.debug(f"📝 使用 Stdio 传输 (命令): {' '.join(server_source)}")
        ...
        log.debug(f"🔍 自动推断传输: {server_source}")
        return server_source

    def _create_transport_from_config(self, config: Dict[str, Any]):
        """从配置字典创建传输"""
        transport_type = config.get("transport", "stdio")
        log.debug(f"⚙️ 使用配置传输类型: {transport_type}")

        if transport_type == "stdio":
            # 检查是否是 Python 脚本
            args = config.get("args", [])
            if args and args[0].endswith(".py"):
                log.debug(f"🐍 配置传输类型:使用 Stdio 传输 (Python), path: {args[0]}, args: {args}")
                return PythonStdioTransport(
                    script_path=args[0],
                ...
                # 使用通用 Stdio 传输
                from fastmcp.client.transports import StdioTransport
                log.debug(f"🐍 配置传输类型:使用 Stdio 传输 (命令), args: {args}")
                return StdioTransport(
            ...
            log.debug(f"🌐 配置传输类型:使用 SSE, url: {config.get('url')}, headers: {config.get('headers')}, auth: {config.get('auth')}")
            return SSETransport(
                url=config["url"],
            ...
            log.debug(f"🌐 配置传输类型:使用 HTTP, url: {config.get('url')}, headers: {config.get('headers')}, auth: {config.get('auth')}")
    ...
    # __aenter__：进入 async with 块时自动执行
    async def __aenter__(self):
        """异步上下文管理器入口"""
        log.debug("🔗 连接到 MCP 服务器...")
