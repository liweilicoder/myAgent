        ...
        **kwargs
        ...
        if key not in cls._instances:
            with cls._lock:
                # 双重检查锁定
                if key not in cls._instances:
                    log.info(f"🔄 创建新的Qdrant连接: {collection_name}")
                    ...
                    log.info(f"♻️ 复用现有Qdrant连接: {collection_name}")
        else:
            log.info(f"♻️ 复用现有Qdrant连接: {collection_name}")
...
class QdrantVectorStore:
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            if self.url and self.api_key:
                ...
                log.success(f"✅ 成功连接到Qdrant云服务: {self.url}")
            elif self.url:
                # 使用自定义URL（无API密钥）
                self.client = QdrantClient(
                    url=self.url,
                ...
                log.success(f"✅ 成功连接到Qdrant服务: {self.url}")
            else:
                # 使用本地服务（默认）
                self.client = QdrantClient(
                    host="localhost",
                ...
                log.success("✅ 成功连接到本地Qdrant服务: localhost:6333")
            ...
            log.error(f"❌ Qdrant连接失败: {e}")
            if not self.url:
                log.info("💡 本地连接失败，可以考虑使用Qdrant云服务")
                log.info("💡 或启动本地服务: docker run -p 6333:6333 qdrant/qdrant")
            else:
                log.info("💡 请检查URL和API密钥是否正确")
            raise
        ...
        try:
            ...
            if self.collection_name not in collection_names:
                ...
                try:
                    ...
                    log.debug(f"跳过更新HNSW配置: {ie}")
            # 确保必要的payload索引
            self._ensure_payload_indexes()

        except Exception as e:
            log.error(f"❌ 集合初始化失败: {e}")
            raise
    ...
    def delete_vectors(self, ids: List[str]) -> bool:
        ...
        try:
            ...
            log.success(f"✅ 成功删除 {len(ids)} 个向量")
            return True
    ...
    def clear_collection(self) -> bool:
        ...
        try:
            ...
            log.success(f"✅ 成功清空Qdrant集合: {self.collection_name}")
            return True
