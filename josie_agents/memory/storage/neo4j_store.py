    ...
    def _initialize_driver(self, **config):
        ...
        try:
            ...
            if "neo4j.io" in self.uri or "aura" in self.uri.lower():
                log.success(f"✅ 成功连接到Neo4j云服务: {self.uri}")
            else:
                log.success(f"✅ 成功连接到Neo4j服务: {self.uri}")
    ...
    def find_related_entities(
        ...
        try:
            ...
            with self.driver.session(database=self.database) as session:
                ...
                log.debug(f"🔍 找到 {len(entities)} 个相关实体")
                return entities

        except Exception as e:
            log.error(f"❌ 查找相关实体失败: {e}")
            return []
    ...
    def search_entities_by_name(self, name_pattern: str, entity_types: List[str] = None, limit: int = 20) -> List[
        ...
        try:
            ...
            with self.driver.session(database=self.database) as session:
                ...
                log.debug(f"🔍 按名称搜索到 {len(entities)} 个实体")
                return entities

        except Exception as e:
            log.error(f"❌ 按名称搜索实体失败: {e}")
            return []
    ...
    def delete_entity(self, entity_id: str) -> bool:
        ...
        try:
            ...
            with self.driver.session(database=self.database) as session:
                ...
                log.info(f"✅ 删除实体: {entity_id} (删除 {deleted_count} 个节点)")
                return deleted_count > 0

        except Exception as e:
            log.error(f"❌ 删除实体失败: {e}")
            return False
    ...
    def clear_all(self) -> bool:
        ...
        try:
            ...
            with self.driver.session(database=self.database) as session:
                ...
                log.info(f"✅ 清空Neo4j数据库: 删除 {deleted_nodes} 个节点, {deleted_relationships} 个关系")
                return True
