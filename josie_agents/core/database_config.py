...
from josie_agents.memory.storage.qdrant_store import QdrantVectorStore
...
class QdrantConfig(BaseModel):
    ...
    def from_env(cls) -> "QdrantConfig":
        ...
        return cls(
            ...
            collection_name=os.getenv("QDRANT_COLLECTION", "josie_agents_vectors"),
...
class DatabaseConfig(BaseModel):
    ...
    def validate_connections(self) -> Dict[str, bool]:
        ...
        try:
            ...
            log.success(f"✅ Qdrant连接验证: {'成功' if results['qdrant'] else '失败'}")
        except Exception as e:
            results["qdrant"] = False
            log.error(f"❌ Qdrant连接验证失败: {e}")
